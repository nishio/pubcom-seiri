...
def group_by_similarity(
    comments: List[str], ids: List[int], similarity_threshold: float = 50.0
    ...
    id_groups = {
        group_idx: [ids[comment_idx] for comment_idx in comment_indices]
...
def generate_similarity_report(
    comments: List[str], groups: List[List[int]], output_dir: str
) -> None:
    """類似度グループのレポートを生成する"""
            ...
            if len(group) > 1:  # 2つ以上のコメントを含むグループのみ表示
                ...
                for j in group[1:]:
                    similarity = calculate_similarity_percentage(
                        comments[representative_idx], comments[j]
                    )
...
def save_results_to_csv(
    ...
    print(
        f"Results saved to {output_dir}/phase1_results.csv and {output_dir}/phase1_results.json"
    )
