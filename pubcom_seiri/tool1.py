...
def perform_clustering(embeddings: np.ndarray, distance_threshold: float = 0.4) -> Tuple[np.ndarray, Dict[str, List[int]]]:
    """凝集クラスタリングを実行する"""
...
def main():
    ...
    # クラスタリングを実行
    labels, clusters = perform_clustering(embeddings, args.threshold)
