...
def parse_args():
    ...
    parser.add_argument(
        "--input",
        required=True,
        help="tool1の出力ディレクトリのパス（clusters.jsonがあるディレクトリ）",
    )
    parser.add_argument(
        "--original", help="元のCSVファイルのパス（除外されていたデータを処理する場合）"
    )
...
def load_excluded_data(
    csv_path: str, included_ids: List[int]
...
def calculate_cluster_centers(
    clusters: Dict[str, List[int]], embeddings: np.ndarray
) -> Dict[str, np.ndarray]:
    """各クラスタの中心を計算する"""
...
def find_closest_to_center(
    cluster_indices: List[int], center: np.ndarray, embeddings: np.ndarray
) -> int:
...
def assign_excluded_data(
    excluded_comments: List[str],
    ...
    # バッチの結果を結合
    excluded_embeddings = (
        np.vstack(excluded_embeddings) if len(excluded_embeddings) > 0 else np.array([])
    )
    ...
    for i, (comment, comment_id) in enumerate(zip(excluded_comments, excluded_ids)):
        ...
        if closest_cluster and min_distance <= threshold:
            ...
            farthest_idx, farthest_dist = find_farthest_from_center(
                cluster_indices, centers[closest_cluster], embeddings
            )
...
def generate_output_data(
    ...
    comments: List[str],
    ids: List[int],
    independent_points: List[int],
    excluded_comments: Optional[List[str]] = None,
    ...
    for idx in independent_points:
        if idx < len(comments):
            # 元のデータからの独立点
            independent_list.append({"comment": comments[idx], "id": ids[idx]})
    ...
    for cluster_id, indices in clusters.items():
        if len(indices) >= 2:  # 2件以上のクラスタのみ処理
            ...
            if closest_idx < len(comments):
                rep_point = {"comment": comments[closest_idx], "id": ids[closest_idx]}
            ...
            for idx in indices:
                if idx != closest_idx and idx < len(comments):
                    cluster_list.append({"comment": comments[idx], "id": ids[idx]})
...
def save_output_data(output_data: Dict[str, Any], output_dir: str) -> None:
    ...
    # 独立点のCSV
    with open(
        f"{output_dir}/independent_points.csv", "w", encoding="utf-8", newline=""
    ) as f:
        writer = csv.writer(f)
    ...
    for cluster_id, cluster_list in output_data["cluster_lists"].items():
        with open(
            f"{output_dir}/cluster_{cluster_id}.csv", "w", encoding="utf-8", newline=""
        ) as f:
            writer = csv.writer(f)
    ...
    # 結合リストのCSV
    with open(
        f"{output_dir}/combined_list.csv", "w", encoding="utf-8", newline=""
    ) as f:
        writer = csv.writer(f)
        writer.writerow(["id", "comment", "type"])
        for point in output_data["independent_points"]:
            writer.writerow([point["id"], point["comment"], "independent"])
        for point in output_data["combined_list"][
            len(output_data["independent_points"]) :
        ]:
    ...
    if duplicates and len(duplicates) > 0:
        ...
        for comment, indices in duplicates.items():
            if len(indices) > 1:
                ...
                duplicate_section += (
                    f"- ID {id_str}は同一内容が{len(indices)}件あった\n"
                )
